...
- If MyList detects that a streaming app is currently active on your Roku, it'll automatically filter search results to titles available on that service. This makes it easy to browse within what you're already watching. You can clear the filter if you'd like to see all results.
...
- **If your Roku is a separate device connected to a TV** (not a Roku TV), consider enabling 1-touch play on both. On the Roku: **Settings → System → Control other devices (CEC) → 1-touch play**. On the TV, look for an HDMI-CEC setting — Samsung calls it Anynet+, LG calls it SimpLink, Sony calls it BRAVIA Sync, but it's the same idea. With both enabled, tapping anything in the MyList remote — or picking up the physical remote — will wake the TV and switch it to the right input automatically. MyList doesn't require it, but it removes a step every time you sit down to watch.
